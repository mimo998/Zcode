...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signup = useCallback(
    async (email: string, password: string, name: string, role: "student" | "teacher") => {
      setLoading(true);
      try {
  ...
  return (
    <AuthShell title="Sign in" onBack={() => navigate("/")}>
      ...
      <p className="mt-5 text-center text-sm text-zinc-500">
        ...
        <button type="button" onClick={() => navigate("/signup")} className="text-emerald-400 hover:text-emerald-300 font-bold">
          Sign up
        </button>
      </p>
    </AuthShell>
...
export function SignupForm() {
  ...
  return (
    <AuthShell title="Create account" onBack={() => navigate("/")}>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <Field placeholder="Name" value={name} onChange={setName} disabled={loading} />
        <Field type="email" placeholder="Email" value={email} onChange={setEmail} disabled={loading} />
        <Field type="password" placeholder="Password" value={password} onChange={setPassword} disabled={loading} />
          ...
          {(["student", "teacher"] as const).map((r) => (
            <button
              ...
              className={`py-2.5 rounded-lg font-bold text-sm transition ${
                role === r ? "bg-zinc-700 text-white shadow-lg" : "text-zinc-400 hover:text-white"
              }`}
            >
      ...
      <p className="mt-5 text-center text-sm text-zinc-500">
        ...
        <button type="button" onClick={() => navigate("/login")} className="text-emerald-400 hover:text-emerald-300 font-bold">
          Sign in
        </button>
      </p>
    </AuthShell>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-zinc-100 px-4 relative overflow-hidden">
      ...
      <div className="relative max-w-md w-full p-8 bg-zinc-900/80 backdrop-blur rounded-2xl border border-zinc-800 shadow-2xl">
        <button onClick={onBack} className="mb-5 text-zinc-400 hover:text-white text-sm font-bold transition">
          ← Back
        </button>
